...
// 2. Fetch submissions for the specified survey
try {
    $stmt = $pdo->prepare("
        SELECT
            s.id,
...
// 3. Get questions for this survey
try {
    $stmt = $pdo->prepare("
        SELECT q.id, q.label
        FROM question q
...
foreach ($submissions as $submission) {
    ...
    foreach ($responses as $response) {
        ...
        $groupedResponses[$questionId][] = $response['response_value'];
    }
    ...
    $row = [
        ...
        $submission['service_unit_name'] ?? 'N/A',
        $submission['location_name'] ?? 'N/A',
        $submission['ownership_name'] ?? 'N/A',
...
for ($i = 1; $i < count($exportData); $i++) {
    ...
    foreach ($headers as $index => $header) {
        $entry[$header] = $exportData[$i][$index] ?? '';
    }
...
switch ($format) {
    case 'pdf':
        ...
        <head>
            <style>
                ...
                .info { margin-bottom: 20px; font-size: 9pt; color: #666; }
                table { width: 100%; border-collapse: collapse; margin-top: 10px; font-size: 9pt; }
                th { background-color: #f2f2f2; padding: 5px; text-align: left; font-weight: bold; border: 1px solid #ddd; }
                td { padding: 5px; border: 1px solid #ddd; vertical-align: top; }
            </style>
        </head>
        <body>
            <h1>' . htmlspecialchars($surveyName) . ' - Survey Data</h1>
    ...
    case 'excel':
        ...
        $spreadsheet->getProperties()
            ->setTitle("Survey Data - " . $surveyName)
            ->setSubject("Survey Responses")
            ->setCreator("Survey Admin");
    ...
    case 'xml':
        ...
        foreach ($structuredData as $submissionData) {
            ...
            foreach ($submissionData as $key => $value) {
                // Ensure valid XML element names (replace spaces with underscores)
    ...
    case 'html':
        ...
        <html>
        <head>
            <title>' . htmlspecialchars($surveyName) . ' - Survey Data</title>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <style>
                body { font-family: Arial, sans-serif; margin: 20px; line-height: 1.6; }
                h1 { color: #2c3e50; margin-bottom: 10px; }
                .info { margin-bottom: 20px; color: #7f8c8d; }
                table { width: 100%; border-collapse: collapse; margin-top: 20px; }
                th { background-color: #3498db; color: white; padding: 10px; text-align: left; }
                td { padding: 8px; border-bottom: 1px solid #ddd; }
                tr:hover { background-color: #f5f5f5; }
                .container { max-width: 1200px; margin: 0 auto; }
                .download-note { margin-top: 30px; font-style: italic; color: #7f8c8d; }
            </style>
        </head>
        <body>
            <div class="container">
        ...
        echo '</tbody>
                ...
                <div class="download-note">
                    Note: You can use the download button to save this data in other formats.
                </div>
            </div>
        </body>
        </html>';
