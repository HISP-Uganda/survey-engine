...
<body style="background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%); min-height: 100vh;">
            ...
            <div class="d-flex justify-content-between align-items-center mt-4 pt-3 border-top">
                <div>
                    <small class="text-muted">
                        ...
                        Tip: You can change question types by clicking on the field type selector
                    </small>
                </div>
                <div>
    ...
    <script>
        ...
        async function fetchLocationsForSurveyPage() {
            ...
            try {
                const response = await fetch(`get_locations.php?survey_id=${surveyId}`);
        ...
        function searchAndDisplayFacilities(searchTerm) {
            ...
            if (matchingFacilities.length > 0) {
                ...
                matchingFacilities.forEach(async (facility) => {
                    try {
                        const response = await fetch(`get_location_path.php?id=${facility.id}`);
                        const data = await response.json();
        ...
        async function selectFacility(facilityId, facilityName, orgunitUid, facilityPath) {
            ...
            try {
                const response = await fetch(`get_location_path.php?id=${facilityId}`);
                const data = await response.json();
        ...
        async function handleSubmit(event) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    window.location.href = 'tracker_program_success.php?survey_id=' + document.getElementById('surveyId').value;
                } else {
        ...
        async function submitAllData() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    setTimeout(() => {
                        window.location.href = `tracker_program_success.php?survey_id=${submissionData.survey_id}&submission_id=${result.submission_id}`;
                    }, 1500);
                } else {
